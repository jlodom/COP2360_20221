...
using System.IO;
using Newtonsoft.Json; /* A library we have imported through NuGet */

namespace Week5A_Example
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            /* Load a list of doubles representing temperature. */
            List<double> listDoubleTemps = LoadTemps();

            /* Convert the list of doubles to a plain text string and write it to a file. */
            String stringTemps = String.Empty;
            foreach (double temp in listDoubleTemps)
            {
                stringTemps += temp.ToString() + Environment.NewLine;
            }
            String stringOutputPath = "examplefile.txt";
            File.WriteAllText(stringOutputPath, stringTemps);


            /* Same thing as writing to the plain text file, but instead we write to a JSON text file. */
            String stringJSONOutputPath = "examplefile2.json";
            String jsonList = JsonConvert.SerializeObject(listDoubleTemps);
            File.WriteAllText(stringJSONOutputPath, jsonList);

            /* More examples showing decisions through if/else */
            foreach (double temp in listDoubleTemps)
            {
                if ((temp >= LOWTEMP) && (temp <= HIGHTEMP))
                {
                    Console.WriteLine("This is a comfortable temp.");
                }
                else if (temp < LOWTEMP)
                {
                    Console.WriteLine("This temp is too low.");
                }
                ...
                {
                    Console.WriteLine("THIS CODE SHOULD NEVER EXECUTE!!!");
                }
                ...
                {
                    case 68:
                        Console.WriteLine("A good year for music.");
                        break;
                    case 73:
                        Console.WriteLine("The 70s really started here.");
                        break;
                    /* Continue in this manner until ... */
                    default:
                        Console.WriteLine("Nothing special about this number.");
                        break;
                }
            }
